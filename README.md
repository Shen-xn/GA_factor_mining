...
当前基线通过97项测试，覆盖无未来信息标签、滚动训练边界、`planned → executed_unsettled → settled`时间轴、交易日历、缺失行情、持仓状态、ETF映射安全门、成本、风险评分、前向协议和运行前自检。
...
本轮在不打开2026的前提下继续验证了健康分固定分档、Top20保留区、8/10日强制持有和连续排名确认。Top20保留区把2018—2025累计收益提高到`+75.2%`、最大回撤改善到`-17.1%`，但换手和5日持有中位数没有改善，不能满足用户要求；加入强制持有或连续确认后虽然换手下降，长期收益或回撤又不过门。正式版本因此仍为`simple_v1`。修正最新执行时间轴和缺报价事件后，当前基准2018—2025累计约`+54.5%`、年化约`5.8%`，平均日换手约`8.9%`，尚未通过收益、低频和成本压力门；`ACCEPTANCE_GATE.json`会明确记录失败项。

当前数据截止2026-08-10。最新收盘计划是30%板块、70%低风险资产，但本地缺少未来交易日历，ETF映射只校验到2026年5月且行情已经过期，因此执行层为`blocked`，`LATEST_ACTIONS.csv`保持为空。该计划只能用于核对状态，不能补单。
