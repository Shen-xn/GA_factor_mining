...
`simple_v2`晋级后又预登记并隔离回放了16条开发期候选，覆盖弱市0—10%仓位、状态确认速度、风险分直接控仓、极端分数覆盖、10日预测目标以及中期人工公式组合。没有候选同时通过2018—2023收益、回撤、换手和多数年份正收益门，因此均未打开2024—2025，也未读取2026作晋级判断。最有信息量的`30/50/70`风险分档把开发期收益提高到`+33.2%`、回撤`-16.2%`、年化换手`12.6`倍，但只有3/6年份为正，仍被拒绝；结构化证据统一放在`outputs/sector/post_v2_research/`。正式策略和v14前向协议没有改变。
...
另一条独立验证只使用财务指标初始公告及公告日，剔除了上市前历史披露；2018—2023月末基本面扩散与未来60日收益的Spearman仅`0.033`，高分组反而少约3.6个百分点，也在策略回放前拒绝。随后把板块选择与市场仓位都限制为每5或10个交易日更新，结果不是换手仍高，就是对起始锚点极不稳定：10日方案在不同锚点下开发期累计从`-1.5%`到`+106.4%`、最大回撤从`-16.4%`到`-29.2%`，因此没有挑选最好日期；`simple_v2`也不依赖固定日历锚点。
