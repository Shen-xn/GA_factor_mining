...
旧项目中“多数年份大幅正收益”的年度LightGBM Top5原型已经重新运行并精确复现，但其收盘标签、次日收益和训练边界与可执行的次日开盘口径不一致，且没有交易成本。统一到当前协议后收益明显下降。按当前`simple_v2`重算的开发期收益桥进一步表明：平滑Top5为`+159.5%`，加入低频持有规则后仍有`+136.6%`；正式市场状态把现金路径降到`+31.1%`并将回撤由`-35.6%`压到`-20.5%`，组合回撤上限只再把收益降至`+30.3%`、回撤改善到`-16.7%`。因此主要收益压缩来自市场状态仓位，而不是Top20/持有10日或组合回撤上限；低风险资产补回收益后，20bp成本把正式开发期结果降至`+23.3%`。“领先板块自身走强时恢复70%仓位”候选又恶化了2018、2022和长期回撤，因此没有进入`simple_v2`。完整证据见板块轮动报告和`outputs/sector/prototype_recovery/`、`outputs/sector/return_bridge/`。
...
`simple_v2`晋级后又预登记并隔离回放了20条开发期候选，覆盖弱市0—10%仓位、状态确认速度、风险分直接控仓、极端分数覆盖、10日预测目标、中期人工公式组合、训练样本时间衰减和持仓置信度加权。没有候选同时通过2018—2023收益、回撤、换手和多数年份正收益门，因此均未打开2024—2025，也未读取2026作晋级判断。最有信息量的`30/50/70`风险分档把开发期收益提高到`+33.2%`、回撤`-16.2%`、年化换手`12.6`倍，但只有3/6年份为正；3年半衰期模型也把开发期收益提高到`+28.2%`，仍只有3/6年份为正。模型分数加权与“模型分数+20日趋势”加权分别只取得`+7.3%`和`+2.5%`，说明分数适合排序、不适合直接作为下注幅度，正式组合继续等权。所有候选均被拒绝；结构化证据统一放在`outputs/sector/post_v2_research/`。正式策略和v14前向协议没有改变。
