...
另一条独立验证只使用财务指标初始公告及公告日，剔除了上市前历史披露；2018—2023月末基本面扩散与未来60日收益的Spearman仅`0.033`，高分组反而少约3.6个百分点，也在策略回放前拒绝。随后把板块选择与市场仓位都限制为每5或10个交易日更新，结果不是换手仍高，就是对起始锚点极不稳定：10日方案在不同锚点下开发期累计从`-1.5%`到`+106.4%`、最大回撤从`-16.4%`到`-29.2%`，因此没有挑选最好日期，也没有建立`simple_v2`。
